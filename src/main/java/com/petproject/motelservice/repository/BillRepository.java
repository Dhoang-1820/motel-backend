...
public interface BillRepository extends JpaRepository<Bills, Integer> {
	...
	@Query(nativeQuery = true, value = "select rooms.id as roomId, rooms.name as roomName, bill.total_payment as totalPayment, bill.paid_money as paidMoney, bill.discount as discount, bill.debt as debt, bill.payment_date as paymentDate, bill.is_pay as isPay, if(bill.id is not null, (select concat(tenants.first_name, ' ',tenants.last_name ) from tenants where id = contract.representative), null) as representative, bill.id as billId from (select rooms.* from rooms join contract on rooms.id = contract.room_id where month(start_date) < month(:month) AND year(start_date) = year(:month)) rooms left join (select * from bills where month(date) = month(:month) AND year(date) = year(:month) and invoice_type_id = 1 and is_active = 1) bill on rooms.id = bill.room_id left join contract on rooms.id = contract.room_id where rooms.accomodation_id = :accomodationId and rooms.is_rent = 1")
	...
	@Query(nativeQuery = true, value = "select rooms.id as roomId, rooms.name as roomName, bill.total_payment as totalPayment, bill.total_price as totalPrice, bill.paid_money as paidMoney, bill.discount as discount, bill.debt as debt, bill.payment_date as paymentDate, bill.is_pay as isPay, if(bill.id is not null, (select concat(tenants.first_name, ' ',tenants.last_name ) from tenants where id = contract.representative), null) as representative, (select end_date from tenants where id = contract.representative) as returnDate, bill.id as billId from rooms join (select * from bills where month(date) = month(:month) AND year(date) = year(:month) and invoice_type_id = 2  and is_active = 1) bill on rooms.id = bill.room_id left join contract on rooms.id = contract.room_id where rooms.accomodation_id = :accomodationId")
