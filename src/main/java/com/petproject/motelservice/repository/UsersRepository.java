...
public interface UsersRepository extends JpaRepository<Users, Integer> {
	...
	@Query(nativeQuery = true, value = "select users.id as userId, users.address, users.username as userName, users.phone, users.email, users.firstname as firstName, users.lasname as lastName, users.identify_num as identifyNum, users.status, users.created_at as createdAt, case when accomodation.accomodationNum > 0 then accomodation.accomodationNum else 0 end as accomodationNum, role.name as role from users left join (select count(accomodations.user_id) as accomodationNum, accomodations.user_id from accomodations group by accomodations.user_id) accomodation on users.id = accomodation.user_id join role on users.role_id = role.id where role.name != 'ROLE_ADMIN'")
	List<UserResponse> findAllUsers();
	
	@Query(nativeQuery = true, value = "select users.id as userId, users.address, users.username as userName, users.phone, users.email, users.firstname as firstName, users.lasname as lastName, users.identify_num as identifyNum, users.status, users.created_at as createdAt, case when accomodation.accomodationNum > 0 then accomodation.accomodationNum else 0 end as accomodationNum, role.name as role from users left join (select count(accomodations.user_id) as accomodationNum, accomodations.user_id from accomodations group by accomodations.user_id) accomodation on users.id = accomodation.user_id join role on users.role_id = role.id where role.name != 'ROLE_ADMIN' and users.status = 'REGISTERED'")
	List<UserResponse> findNewRegisterUsers();
